for /F "tokens=1,2,3 delims=," %%A in (..\other\regions.csv) Do (
 ...
 if not exist extracts\%%A.poly ..\executable\getbound.pl --onering --noinner --api=op_ru -o extracts\%%A.poly %%B
 if not exist extracts\exp-%%A.poly ..\executable\offset\offset.exe extracts\%%A.poly extracts\exp-%%A.poly
 if not exist extracts\%%A.osm ..\executable\osmconvert ../UA.osm.pbf -B=extracts\exp-%%A.poly  -o=extracts\%%A.osm
