for /F "tokens=1,2,3 delims=," %%A in (..\other\regions.csv) Do (
 ...
 if not exist extracts\%%A.poly perl ..\executable\getbound.pl --onering --noinner --api=op_ru --offset=0.005 -o extracts\%%A.poly %%B
REM if not exist extracts\exp-%%A.poly ..\executable\offset\offset.exe extracts\%%A.poly extracts\exp-%%A.poly
 ...
 if not exist maps\Navitel-%%A.mp perl ..\executable\osm2mp\osm2mp.pl --config=..\executable\osm2mp\cfg-navitel\navitel-uk.cfg  --target-lang=uk --default-lang=uk  --defaultcountry="UA" -o maps\Navitel-%%A.mp extracts\%%A.osm
 if not exist maps\Navitel-%%A.mp.err.htm perl ..\executable\osm2navitel\mp-postprocess-navitel.pl maps\Navitel-%%A.mp --fixrouting
)
